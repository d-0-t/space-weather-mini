...
6. As a visitor, I want a chart next to each table (Kp history timeline, 27-day radio flux/A index trend, 3-day Kp forecast line), so that I can see trends the table hides.
...
- **Rendering**: semantic HTML tables for all tabular products, styled with SCSS + BEM (`block__element--modifier`, e.g. `list__item__img`; nesting only within BEM structure). Recharts visualisations (Kp history timeline, 27-day radio flux/A index trend, 3-day Kp forecast line) render beside the tables carrying the same data — the table is the accessibility source of truth. The `.kp01`–`.kp9` color classes are preserved as the sole design-token mechanism; no inline hex in components.
...
- **Vocabulary**: code, types, and user-facing copy follow CONTEXT.md. The geophysical alert product is named `geophysical-alert` internally with the NOAA label "Geophysical Alert Message" in the UI; the `regionale` typo is gone — the 3-day forecast's per-section prose is its "Rationale", not a "Regional text" (the current NOAA product carries no per-region prose, verified against the 2019/2023/2026 archives); `geospace` is used only for the discussion section, never as a synonym for geomagnetic activity.
