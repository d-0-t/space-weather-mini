...
## Comments (2026-09-06 follow-up, on-demand glow table)

After the ticket was committed (3a3a604, including the user's probability-card redesign of the band line), the visual oval map gained its table alternative per the no-sr-only-tables rule: countGlowLevels is back (pure, boundary rows excluded so the numbers match the painted map) and renders as a closed-by-default `<details>` disclosure between the 30-90 min lead line and the map - summary reads Glow intensity table with a decorative ReadMore icon; opening it reveals a real, visible Glow level / North cells / South cells table for everyone. Never sr-only. Tests: disclosure placement + default-closed + on-demand visibility + per-hemisphere counts with boundary rows excluded. Docs updated (coding-standards realized pattern, ADR-0006/0007, CONTEXT Oval term). Three committed tests still referencing the pre-redesign .view-distance__place markup were aligned to the probability card. Suite: 	sc --noEmit clean, Vitest 648 green, build emits sw.js. Not committed.

- Follow-up (2026-09-06, user): the oval freshness line now reads `Forecast Time Sep 4 14:33 UTC (16:33 your time) - 30-90 min lead.` - `formatLocalShort` renders the device-local HH:MM beside the UTC reference and includes the local date when the zone crosses midnight, so a forecast time a few minutes ahead of now no longer reads as wrong. Suite: tsc clean, Vitest 651 green, build emits sw.js. Not committed.

- Follow-up (2026-09-06, user, ticket 06 review round): the line's `Change location` button is now **icon-only** - `title` + sr-only span + the location icon, no visible label text - per the icon-only rule; the band text and `(i)` popover are unchanged. Implemented and tested with ticket 06.
