...
**Reach towns**:
The Possible locations panel's global town list, headed "Possible locations" (earlier "Towns where it may be visible"): one flag, town name and ranked probability per row, plus a camera icon on towns that own a webcam. A town appears only while its approximate |geomagnetic latitude| is at or poleward of the Tips reach edge (66° − 2° × Kp) and the sun there is at or below −12° (astronomical twilight or darker). Webcam towns (the image and live cams' towns) always appear once they qualify — their camera icon must never be crowded out — while the other guide towns keep the diversity rule: at most one town per probability per country (largest margin wins, then alphabetical), those guide rows capped at 12. All rows order Very likely first; nothing renders when no town qualifies. The probability is one of three ordinal words — Possible / Likely / Very likely — never a percentage; the icon ranks them by filled bars and its colour is redundant. The list is an approximate average in geomagnetic latitude, not geographic, and never a per-town promise.
