...
- **Type tokens (ADR-0009)**: every `font-size` uses a semantic Type token from `src/styles/_typography.scss` (`--font-size-caption/small/body/lead/h1–h4`, each with a paired `--line-height-*` token applied as two declarations). Headings h1–h4 are sized once by the global element rules in that partial — a new page needs zero sizing CSS, just semantic headings — and the body element carries the body token. Width media queries go through `@include respond-to(sm|md|lg)` from `src/styles/_breakpoints.scss` (the canonical Breakpoints: sm 480px, md 810px, lg 1100px, mobile-first min-width only; `@use` the breakpoints partial at the file's own depth). A legacy max-width rule inverts mobile-first: the narrow state becomes the default and `respond-to` restores the wide state (the nav's hamburger panel is the default; `respond-to(lg)` restores the wide bar). Raw rem/em font-sizes and raw-pixel width media queries are forbidden; the frozen data-chrome literals are the documented exceptions — the Aurora Now Kp readout, the live-panel value family, the moon emoji badge and webcam pin glyphs, the Local conditions temperature readout (with its 1.4em weather icon and relative detail-icon `em`), the CollapsiblePanel's below-sm panel-title step to the h3 token, plus the tuned data-chrome line-heights (mini-table day row, Kp bar segment, live-panel values, webcams prose notes, weather hour-card box, icon-button/glyph boxes) — and non-width queries (`orientation`, `prefers-reduced-motion`) stay raw.
