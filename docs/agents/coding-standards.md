...
- **Shared button system** (ticket 05): `.btn--primary` / `.btn--secondary` plus `.btn__label` live in `index.scss` as a deliberate cross-block utility layer (the BEM exception — buttons on any page may adopt the modifier classes directly, as the Astro toggle, browser-alerts button, Compact view toggle and all webcams controls do). `.btn--secondary` is the dark 44px pill (formerly `.btn--icon`); icon-only controls (close, help "?", toolbar buttons below 1000px) are named by `title` + a `.sr-only` span, never `aria-label`.
