...
- **Type tokens (ADR-0009)**: every `font-size` uses a semantic Type token from `src/styles/_typography.scss` (`--font-size-caption/small/body/lead/h1–h4`, each with a paired `--line-height-*` token applied as two declarations). Headings h1–h4 are sized once by the global element rules in that partial — a new page needs zero sizing CSS, just semantic headings — and the body element carries the body token. Width media queries go through `@include respond-to(sm|md|lg)` from `src/styles/_breakpoints.scss` (the canonical Breakpoints: sm 480px, md 810px, lg 1100px, mobile-first min-width only; the file needs `@use "../../styles/breakpoints" as *;` from its own depth). Raw rem/em font-sizes and raw-pixel width media queries are forbidden; the frozen data-chrome literals (the Aurora Now Kp readout, the live-panel value family, the moon emoji badge and webcam pin glyphs, plus the tuned data-row line-heights) are the documented exceptions, and non-width queries (`orientation`, `prefers-reduced-motion`) stay raw.
