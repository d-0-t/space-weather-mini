...
import { WEBCAM_TOWN_KEYS } from "../../../../../data/webcams";
import {
  selectReachTowns,
  type ReachTown,
 ...
 * empty-vs-list decision and the rendered rows never disagree. Webcam towns
 * ride in past the guide dedup (WEBCAM_TOWN_KEYS) so their camera icons
 * always show when the aurora reaches them.
 */
export function useReachTowns(kp: number | null): ReachTown[] {
  ...
  return useMemo(() => {
    if (kp === null) return [];
    return selectReachTowns(
      REACH_CITIES,
      kp,
      (entry) => solarElevationDegrees(entry.lat, entry.lon, now),
      WEBCAM_TOWN_KEYS,
    );
  }, [kp, now]);
}
