...
describe("live-panels chart helpers", () => {
  it("computes L1 transit time and minute-shifted Now tags", () => {
  ...
  it("averages rows into time buckets for a cleaner line", () => {
    ...
    expect(points[1].timeTag).toBe(
      new Date("2026-08-26T22:05:00Z").toISOString(),
    );
  ...
  it("places the Now anchor transit points LEFT of the freshest reading", () => {
    const points = [
      { x: 179, timeTag: "2026-08-26T22:00:00", value: 1 },
    ];
    const anchored = withNowAnchor(points, "2026-08-26T20:32", 88);
    expect(anchored).toHaveLength(2);
    expect(anchored[1]).toEqual({
      x: 91,
      timeTag: "2026-08-26T20:32",
      value: null,
    });
    // No anchor when a reading already sits in the Now minute (1-min feeds
    // cover it) – the match is instant-based, not a label comparison.
    expect(
      withNowAnchor(points, "2026-08-26T22:00:30", 88),
    ).toHaveLength(1);
  });
