...
class Parser {
  ...
  DateParser(dateNasaUTC, colonPosition) {
    ...
    return [dateNasaUTC.slice(0,colonPosition), ":", dateNasaUTC.slice(colonPosition,dateNasaUTC.length)].join("");
  }
  ...
  DiscussionParser(discussion) {
    ...
    let split1 = [ ":Product: ", "Solar Activity", "Energetic Particle", "Solar Wind", "Geospace" ];
    ...
    for (let i = 0; i < filteredString[0].length; i++) {
      filteredString[0][i] = filteredString[0][i].replaceAll(split4[0], "").replace(split4[1], "");
    }
  ...
  GeoAlertParser(geoAlert) {
    let split1 = [ ":Product: ", ":Issued: ", "<br/># ", "<br/>#<br/>#", "<br/>#<br/>" ];
  ...
  TwentySevenDays(xdays) {
    ...
    function tableMaker27() {
      ...
      while (count < tableData.length) {        
        rows +=
        '<tr kp-value="' + tableData[count][3] + '"><td className="tableDate">' + tableData[count][0]
        + "</td><td className='radioFlux'>"  + tableData[count][1]
        + "</td><td className='indexA'>"  + tableData[count][2]
        + "</td><td className='indexKp'>"  + tableData[count][3]
        + "</td></tr>"
        count++;
      }
      return rows;
    ...
    let htmlTable27 = "<table id='table27days'>" + tableHead27 + "<tbody>" + tableMaker27() + "</tbody></table>";
  ...
  ThreeDays(three) {
    let split1 = [ ":Product: ", "<br/>:Issued: ", "<br/># ",
    "<br/>#<br/>A. NOAA ", "<br/><br/>B. NOAA ", "<br/><br/>C. NOAA " ];
    ...
    let table_kp = filteredString[3].splice(2,2)
                .join("<TITLE_BR>").split("<TITLE_BR>");
    table_kp[1] = table_kp[1].replace(" 00-03UT","<HEAD>00-03UT").split("<HEAD>");
    ...
    for (let i = 0; i < 32; i += 4) {
      table_kp_body[i/4]
      .push(table_kp[1][1][i+0], table_kp[1][1][i+1],
            table_kp[1][1][i+2], table_kp[1][1][i+3]);
    }
    ...
    table_radio[1][1] = table_radio[1][1].replace(" R3","<SPLIT>R3").split("<SPLIT>");
    ...
    <th className="tableHeader" scope="col">` + table_kp[1][0][1] + `</th>
    <th className="tableHeader" scope="col">` + table_kp[1][0][2] + `</th>
    <th className="tableHeader" scope="col">` + table_kp[1][0][3] + `</th>
    </tr></thead>`;
    function tableMaker3Kp(tableData) {
      ...
      while (count < tableData.length) {        
        rows +=
        '<tr><td value="' + tableData[count][0] + '" className="date">' + tableData[count][0]
        + '</td><td kp-value="' + tableData[count][1] + '" className="valAtDate">'  + tableData[count][1]
        + '</td><td kp-value="' + tableData[count][2] + '" className="valAtDate">'  + tableData[count][2]
        + '</td><td kp-value="' + tableData[count][3] + '" className="valAtDate">'  + tableData[count][3]
        + '</td></tr>'
        count++;
      }
      return rows;
    }
    let htmlTable3Kp = "<table id='table3days_kp'>" + htmlTable3KpHead
    + "<tbody>" + tableMaker3Kp(table_kp[1][1]) + "</tbody></table>";

    // Construct html table 2
    let htmlTable3SrsHead =
    `<thead><tr>
    <th className="tableHeader" scope="col"></th>
    <th className="tableHeader" scope="col">` + table_srs[1][0][1] + `</th>
    <th className="tableHeader" scope="col">` + table_srs[1][0][2] + `</th>
    <th className="tableHeader" scope="col">` + table_srs[1][0][3] + `</th>
    </tr></thead>`;
    let htmlTable3SrsBody = 
    '<tbody><tr><td rowTitle="' + table_srs[1][1][0] + '" className="valAtDate">' + table_srs[1][1][0] + 
    '</td><td percent="' + table_srs[1][1][1] + '" className="valAtDate">'  + table_srs[1][1][1] + 
    '</td><td percent="' + table_srs[1][1][2] + '" className="valAtDate">'  + table_srs[1][1][2] + 
    '</td><td percent="' + table_srs[1][1][3] + '" className="valAtDate">'  + table_srs[1][1][3] +
    '</td></tr></tbody>';
    let htmlTable3Srs = "<table id='table3days_srs'>"
    + htmlTable3SrsHead + htmlTable3SrsBody + "</table>"

    // Construct html table 3
    let htmlTable3radioHead =
    `<thead><tr>
    <th className="tableHeader" scope="col"></th>
    <th className="tableHeader" scope="col">` + table_radio[1][0][1] + `</th>
    <th className="tableHeader" scope="col">` + table_radio[1][0][2] + `</th>
    <th className="tableHeader" scope="col">` + table_radio[1][0][3] + `</th>
    </tr></thead>`;
    function tableMaker3Radio(tableData) {
      ...
      while (count < tableData.length) {        
        rows +=
        '<tr><td rowTitle="' + tableData[count][0] + '" className="valAtDate">' + tableData[count][0]
        + '</td><td percent="' + tableData[count][1] + '" className="valAtDate">'  + tableData[count][1]
        + '</td><td percent="' + tableData[count][2] + '" className="valAtDate">'  + tableData[count][2]
        + '</td><td percent="' + tableData[count][3] + '" className="valAtDate">'  + tableData[count][3]
        + '</td></tr>'
        count++;
      }
      return rows;
    }
    let htmlTable3radio = "<table id='table3days_radio'>" + htmlTable3radioHead
    + "<tbody>" + tableMaker3Radio(table_radio[1][1]) + "</tbody></table>";

    return [ filteredString, /*table_kp, table_srs, table_radio,*/
             htmlTable3Kp, htmlTable3Srs, htmlTable3radio ];
  }
  ...
  DailyParser(daily) {
    ...
    for (let i = 0; i < filteredString[3][1].length-1; i++) {
      ...
      let dateSplit = filteredString[3][1][i].match(/[\d]{4}[ ][\d]{2}[ ][\d]{2}/);
    ...
    function tableSubHeaderConstructor(subHeadArray) {
      ...
      while (counter < subHeadArray.length) {
        ...
        subHead += '<td cellType="tableSubHeader" colspan="' + colspan + '">' + subHeadArray[counter] + '</td>';
        counter++;
      }
      return subHead + '</tr>';
    }
    ...
    function htmlTableBodyConstructor(bodyArray) {
      ...
      while (i < bodyArray.length) {
        ...
        while (j < bodyArray[i].length) {
          ...
          bodyOfTable += '<td cellType="tableBody" ' + valueIndicator + '="' + Math.abs(bodyArray[i][j]) + '">' + bodyArray[i][j] + '</td>';
          j++;
        }
        bodyOfTable += "</tr>";
        i++;
