...
describe("parseForecastDiscussion", () => {
  ...
  it("parses the four sections with typed summary and forecast prose", () => {
    ...
    expect(discussion.solarActivity.forecast).toBe(
      "Solar activity is expected to be at low levels, with a chance for M-flares (R1-R2/Minor-Moderate) and a slight chance for an X-flare (R3/Strong or greater) over 23-25 Aug."
    );
    expect(discussion.energeticParticle.daySummary).toBe(
      "The greater than 2 MeV electron flux reached high levels. The greater than 10 MeV proton flux remained elevated, ranging from 2-5 pfu throughout the period."
    );
    ...
    expect(discussion.geospace.daySummary).toBe(
      "The geomagnetic field was at quiet levels under an ambient solar wind environment."
    );
  });

  it("keeps paragraph breaks and sentence-boundary breaks, joining mid-sentence wraps", () => {
    const { solarActivity } = parseForecastDiscussion(fixture);
    expect(solarActivity.daySummary).toBe(
      "Solar activity remained at low levels. Region 4513 (N04E32, Eai/beta-gamma) produced multiple C-flares, the largest being a C4.1/Sf at 23/0653 UTC, and remained stable throughout the period. Region 4517 (N10E18, Cao/beta) exhibited minor growth, while Regions 4508 (N08, L=166) and 4511 (S09, L=148) decayed to plage.\n\nOther activity included an area of coronal dimming near S20E04 at 22/2030 UTC, but no associated CME was detected in coronagraph imagery.\nAt around 22/0030 UTC, a filament lifted off near N40E45, and produced a narrow CME that is well north of the Sun-Earth line. No Earth-directed CMEs were detected in available coronagraph imagery."
    );
  });
